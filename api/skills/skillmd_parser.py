...
_CROSS_SKILL_LOOKUPS = {
    "company": ("erpclaw", "list-companies"),
    "account": ("erpclaw", "list-accounts"),
    "fiscal-year": ("erpclaw", "list-fiscal-years"),
    "cost-center": ("erpclaw", "list-cost-centers"),
    "item": ("erpclaw", "list-items"),
...
def _parse_flags(cell: str, required: bool) -> list[dict]:
    ...
    fields = []
    # Strip outer backticks if present (e.g. `--flag1 --flag2`)
    stripped = cell.strip("`").strip()

    # Detect space-separated --flag tokens (e.g. "--name --company-id --city")
    if re.search(r"--\w[\w-]*\s+--\w[\w-]*", stripped):
