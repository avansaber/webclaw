...
async def audit_middleware(request: Request, call_next):
    ...
    loop.run_in_executor(None, partial(_write_audit_log, user_id, skill, action, response.status_code, request_id))

    return response


def _write_audit_log(user_id: str | None, skill: str, action: str, status_code: int, request_id: str | None = None):
    ...
    try:
        ...
        conn = get_connection()
        conn.execute(
            """INSERT INTO audit_log (id, user_id, skill, action, description, request_id, created_at)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (
                str(uuid.uuid4()),
                user_id,
