...
async def chat_stream(request: Request):
    ...
    # Inject adaptive vocabulary context into system prompt
    try:
        vocab_ctx = get_vocabulary_context(user_id)
        if vocab_ctx:
            context["adaptive_profile"] = vocab_ctx
    except Exception:
        pass  # Non-critical — don't break chat

    # Auto-resolve entities from message if none provided (E1)
    ...
    if write_intent:
        ...
        async def composition_generator():
            ...
            _save_message(conn, session_id, "assistant", composition_text, context)
