...
// Runtime cache maps list actions to the skill that owns the entity.
// Populated dynamically from the action-index API endpoint.
// This replaces the previous hardcoded ACTION_SKILL_MAP.

let _actionSkillCache: Record<string, string> = {};
let _cachePopulated = false;

/** Populate the action→skill cache from the action-index API. Call once on app load. */
export async function populateActionSkillCache(): Promise<void> {
  if (_cachePopulated) return;
  try {
    const res = await fetch("/api/v1/schema/action-index");
    if (!res.ok) return;
    const data = await res.json();
    const actions = data.actions as { action: string; skill: string }[] | undefined;
    if (actions) {
      const map: Record<string, string> = {};
      for (const entry of actions) {
        map[entry.action] = entry.skill;
      }
      _actionSkillCache = map;
      _cachePopulated = true;
    }
  } catch {
    // Silently fail — forms will still work, just without cross-skill lookups
  }
}

/** Inject entries directly (e.g. from pre-fetched action index data). */
export function setActionSkillCache(entries: { action: string; skill: string }[]): void {
  for (const entry of entries) {
    _actionSkillCache[entry.action] = entry.skill;
  }
  _cachePopulated = true;
}

function resolveEntitySkill(searchAction: string): string | undefined {
  return _actionSkillCache[searchAction];
}
...
// Get all list actions with their entity info (filters out tab_hidden actions)
export function getListActions(config: UIConfig): { action: string; entity: string; label: string; addAction?: string; domain?: string }[] {
  // Build entity→domain lookup from domains config
  const entityDomainMap: Record<string, string> = {};
  if (config.domains) {
    for (const domain of config.domains) {
      for (const entityKey of domain.entities) {
        entityDomainMap[entityKey] = domain.key;
      }
    }
  }

  return Object.entries(config.action_map || {})
    .filter(([, entry]) => entry.component === "DataTable" && !entry.tab_hidden)
    .map(([action, entry]) => {
      ...
      return {
        ...
        domain: entityDomainMap[entry.entity || ""],
      };
    });
}
