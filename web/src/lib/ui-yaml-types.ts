...
export interface KpiDef {
  ...
  action: string;                     // list action to call (e.g. "list-units")
  filter?: Record<string, string>;    // query params (e.g. { status: "vacant" })
  field?: string;                     // response field to read (default: "total_count")
  total_field?: string;               // denominator field for "X of Y" display
  total_action?: string;              // separate action for denominator (if different)
  severity?: "info" | "warning" | "success";
  drill_action?: string;              // entity list action to navigate to on click
  drill_filter?: Record<string, string>;
...
export interface DashboardConfig {
  kpis: KpiDef[];
  quick_actions?: QuickActionDef[];
}

// ── Workflow Hints (post-action suggestions) ────────────────────────────

export interface WorkflowSuggestion {
  action: string;
  label: string;
  skill?: string;                    // Cross-skill target (defaults to current skill)
  pass?: Record<string, string>;     // Map: target param → source field from response
}

export interface WorkflowRule {
  after: string;                     // Action name that triggers this rule
