...
export async function fetchApi(
  ...
  // Extract request ID for error debugging
  const requestId = res.headers.get("X-Request-ID");

  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    const msg = (body as Record<string, string>).message || (body as Record<string, string>).error || `API error ${res.status}`;
    const err = new Error(msg);
    (err as Error & { requestId?: string }).requestId = requestId ?? undefined;
    throw err;
  }
...
// ── Action index (for Cmd+K) ────────────────────────────────────────────────

export interface ActionIndexEntry {
  action: string;
  skill: string;
  type: string;     // "list" | "add" | "create" | "get" | "update" | "submit" | "cancel" | "delete" | "other"
  group: string;    // entity group name (e.g. "Customers", "Properties")
}

let _actionIndexCache: { entries: ActionIndexEntry[]; fetchedAt: number } | null = null;

export async function getActionIndex(): Promise<ActionIndexEntry[]> {
  if (_actionIndexCache && Date.now() - _actionIndexCache.fetchedAt < 5 * 60 * 1000) {
    return _actionIndexCache.entries;
  }
  const data = await fetchApi("/schema/action-index");
  const entries = (data.actions as ActionIndexEntry[]) || [];
  _actionIndexCache = { entries, fetchedAt: Date.now() };
  return entries;
}
...
> = {
  ...
  "property-management": { label: "Property Mgmt", order: 18 },
