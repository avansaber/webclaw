...
} from "@/components/profile-selector";
...
export function OnboardingWizard({
  ...
  // Step 3: Business profile selection
  ...
  // Load skills and profile templates for step 3
  useEffect(() => {
    getSkills()
      .then(setSkills)
      .catch(() => {})
    ...
    fetch("/api/v1/adaptive/profiles")
      .then((r) => r.json())
      .then((data) => {
        if (data.status === "ok" && Array.isArray(data.profiles)) {
  ...
  async function handleCompanySetup() {
    ...
    try {
      ...
      if (res.status !== "ok") {
        ...
        const listRes = await fetchApi("/erpclaw/list-companies", {
          method: "GET",
        });
        if (listRes.status === "ok" && Number(listRes.total_count) > 0) {
          // Company already exists — use the first company's ID
          const companies = listRes.companies as Array<{ id: string }>;
          if (companies?.[0]?.id) {
      ...
      if (res.company_id) {
        setCreatedCompanyId(res.company_id as string);
      }
      setCompanyCreated(true);
      setCompanySubmitting(false);
      goToStep(3);
  ...
  async function handleDemoData(): Promise<boolean> {
    ...
    try {
      const seedBody: Record<string, string> = {};
      if (createdCompanyId) {
        seedBody["company-id"] = createdCompanyId;
      }
      ...
      // Accept both "ok" and "partial" (partial = some non-critical items failed)
      ...
      setDemoProgress(
        res.status === "partial"
          ? "Demo data loaded (some optional items skipped)"
      ...
      clearInterval(interval);
      const msg = e instanceof Error ? e.message : "Unknown error";
      setDemoError(`Failed to load demo data: ${msg}`);
      setDemoLoading(false);
      return false;
    }
  }
  ...
  async function handleNext() {
    if (currentStep === 2) {
      ...
      if (selectedProfile) {
        // Activate the selected profile via API
        setProfileActivating(true);
        try {
          await fetchApi("/adaptive/profiles/activate", {
            method: "POST",
            body: JSON.stringify({
              profile_key: selectedProfile,
              extra_skills: selectedExtras,
            }),
          });
        } catch {
  ...
  function handleToggleExtra(skill: string) {
  ...
  const selectedTemplate = profiles.find((p) => p.key === selectedProfile);
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          ...
          <h3 className="text-xl font-semibold">What are you building?</h3>
          <p className="text-sm text-muted-foreground mt-1">
            Choose your business type — we&apos;ll configure the right modules
          </p>
        </div>
        ...
        {/* Show skill breakdown for selected profile */}
        {selectedTemplate && (
          <SkillToggleGrid
            coreSkills={selectedTemplate.core_skills}
            optionalSkills={selectedTemplate.optional_skills}
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {selectedTemplate
                  ? selectedTemplate.core_skills.length + selectedExtras.length
                  : skills.filter(
          ...
          <CardContent className="p-6">
            ...
            <div className="flex items-center justify-between mt-8 pt-4 border-t">
              ...
              <div className="flex gap-2">
                ...
                >
                  {companySubmitting || demoLoading || profileActivating ? (
                    <>
